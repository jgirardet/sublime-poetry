...
class TestConfig(PoetryDeferredTestCase):
    ...
    def test_modif_credential(self):
        self.pc._callback_credentials = MagicMock()

        self.pc.window.show_quick_panel = MagicMock(side_effect = [lambda v, w, x, y: self.pc.dispatch(
            self.choose("Configure cred")), lambda: self.pc._callback_credentials()])

        self.pc.run()
        self.assertTrue(self.pc._callback_credentials.called)
